...
if [[ "$OS" == "Linux" ]]; then
  ...
  if [[ "$CURRENT_PORT" == "22" ]]; then
    echo ""
    warn "当前 SSH 端口为 22，建议改成非默认端口"
    read -rep "$(echo -e "   ${BLUE}▸${NC} 输入新 SSH 端口（默认 19822，回车采用默认）: ")" PRECONF_NEW_PORT
    PRECONF_NEW_PORT="${PRECONF_NEW_PORT:-19822}"
    if ! [[ "$PRECONF_NEW_PORT" =~ ^[0-9]+$ ]] || [[ "$PRECONF_NEW_PORT" -lt 1024 || "$PRECONF_NEW_PORT" -gt 65535 ]]; then
      fail "端口号无效（需要 1024-65535）"
    fi
    PRECONF_NEED_SSH_CHANGE=true
  fi
fi
...
if [[ "$NEED_GH_VERIFY" == "true" || "$PRECONF_NEED_SSH_CHANGE" == "true" || "$OS" == "Linux" ]]; then
  ...
  if [[ "$NEED_GH_VERIFY" == "true" ]]; then
    PRECONF_TODO_IDX=$((PRECONF_TODO_IDX + 1))
    echo ""
    echo -e "   ${BOLD}${PRECONF_TODO_IDX}. 把下面这个公钥贴到 GitHub${NC}"
    echo -e "      地址：${BLUE}https://github.com/settings/ssh/new${NC}"
    echo -e "      Title：${BOLD}omps-$(hostname)${NC}"
    echo ""
  ...
  if [[ "$OS" == "Linux" ]]; then
    ...
    if [[ "$PRECONF_NEED_SSH_CHANGE" == "true" ]]; then
      echo -e "   ${BOLD}${PRECONF_TODO_IDX}. 在云控制台安全组放行（入方向 TCP）${NC}"
      echo -e "      ${BOLD}${PRECONF_NEW_PORT}${NC}    SSH 新端口（不放行 sshd 重启后会失联）"
      echo -e "      ${BOLD}80${NC}        Nginx Gateway（前端 / Cloudflare 回源）"
    else
      echo -e "   ${BOLD}${PRECONF_TODO_IDX}. 确认云控制台安全组已放行（入方向 TCP）${NC}"
      echo -e "      ${BOLD}80${NC}        Nginx Gateway（前端 / Cloudflare 回源）"
    fi
...
if [[ "$OS" == "Linux" ]]; then
  ...
  if [[ "$PRECONF_NEED_SSH_CHANGE" == "true" ]]; then
    NEW_PORT="$PRECONF_NEW_PORT"
    info "修改 SSH 端口 ${CURRENT_PORT:-22} → $NEW_PORT"
...
# ── 3. SSH Key ──
# 已在前置配置阶段生成 + 验证，这里只是占位记录

step "确认 SSH Key"
ok "SSH Key 已就绪 + GitHub SSH 已连通"
...
# 用 || TEMP_RESULT="" 兜底：set -e 模式下，如果 curl 任何非零退出（404 / 已存在账号 / 网络抖）
# 命令替换的失败会让整个脚本提前 exit，导致后面的摘要永远不打印
TEMP_RESULT=$(curl -sf -X POST "http://localhost:80/$API_PREFIX/api/auth/create-temp-account" -H "Content-Type: application/json" 2>/dev/null) || TEMP_RESULT=""
if [[ -n "$TEMP_RESULT" ]]; then
  TEMP_USER=$(echo "$TEMP_RESULT" | python3 -c "import sys,json; d=json.load(sys.stdin); print(d.get('data',{}).get('username',''))" 2>/dev/null || true)
  TEMP_PASS=$(echo "$TEMP_RESULT" | python3 -c "import sys,json; d=json.load(sys.stdin); print(d.get('data',{}).get('password',''))" 2>/dev/null || true)
  ENTRY_PATH=$(echo "$TEMP_RESULT" | python3 -c "import sys,json; d=json.load(sys.stdin); print(d.get('data',{}).get('entry_path',''))" 2>/dev/null || true)
fi
...
# 外部部署：admin-web GitHub Variables 配置指引
#
# 实际流程：push → GitHub Actions 用 vars/secrets.VITE_API_BASE 跑 npm run build
# → wrangler 把 dist 上传到 Cloudflare Pages（CF 端不 build）
# 所以变量要设在 GitHub 仓库的 Variables，不是 CF Dashboard
if [[ "$WEB_MODE" == "external" ]]; then
  echo ""
  echo -e "   ${YELLOW}┌────────────────────────────────────────────┐${NC}"
  echo -e "   ${YELLOW}│  配置 admin-web 配置                       │${NC}"
  echo -e "   ${YELLOW}├────────────────────────────────────────────┤${NC}"
  echo -e "   ${YELLOW}│${NC}   访问 https://github.com/oh-my-porjects/omps-admin-web/settings/variables/actions"
  echo -e "   ${YELLOW}│${NC}"
  echo -e "   ${YELLOW}│${NC}   Variable :  ${BOLD}VITE_API_BASE${NC}"
  echo -e "   ${YELLOW}│${NC}   Value:          ${BOLD}https://${ADMIN_DOMAIN}/${API_PREFIX}${NC}"
  echo -e "   ${YELLOW}│${NC}"
  echo -e "   ${YELLOW}│${NC} 加完后重新触发 action"
  echo -e "   ${YELLOW}└────────────────────────────────────────────┘${NC}"
fi
